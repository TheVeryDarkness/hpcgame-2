...
代码仓库将放在<https://github.com/TheVeryDarkness/hpcgame-2.git>，Writeup 提交截止一周左右会公开。
...
仓库里会有当时用的 [Jupyter Notebook](./b.ipynb)。
...
从类型论的角度来说，如果这两者兼容的话，可以往 `void **` 指向的地方写入 `const void *`，然后再作为 `void *` 读出来，unsound。
...
- Fortran 默认按引用传参的，有几个整数或者浮点数类型的参数需要指定为按值传递。
...
~~虽然我觉得多进程并行非常难用，但是我感觉这题还挺有意思，写 Writeup 的时候终于改到能通过示例的程度了。希望 POD 先别关。~~

## 雷方块

~~写了很久只拿到 10 分，发现自己还是太年轻了，错把示例当成宝。~~
...
我感觉我这种做法是在基于标准库的排序实现的情况下比较好的了，但是似乎 Julia 标准库中的排序的性能并没有到达极限。我一方面偷了懒，没有思考怎么实现基数排序，另一方面是思路没有放开，没有想着自己实现排序。
...
Copliot 的补全也没有记录，我直接把初版代码贴在下面吧。
...
测试了很多次，比如说改传参类型、改计算时使用的精度，但是神秘的物理学发挥作用了，干涉本身就是很多波函数互相抵消导致的，很小的误差叠加起来导致的误差也会非常大。

最后还是把 `cos` 和 `sin` 换成了 `cossin`，才多拿了 4%。
...
这次没太多时间做，加上基础不够好，走了很多弯路，成绩不是很理想。不过这次的题目确实很有意思，也学到了很多东西。
